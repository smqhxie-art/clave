...
export function HistorySidebarSection() {
  ...
  return (
    <div className="pb-3">
      <SectionHeading
        title={isSearchMode ? t('history.sidebar.heading.search') : t('history.sidebar.heading.default')}
        ...
        actions={
          <button
            ...
            title={t('history.sidebar.refreshTooltip')}
          >
        ...
        <CollapsibleContent className="overflow-hidden data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=closed]:animate-out data-[state=closed]:fade-out-0">
          <div className="px-2 space-y-1">
            {isSearchMode && (
              <div className="px-2.5 mb-2">
                <div className="mb-2 text-[11px] text-text-tertiary">
                  {t('history.sidebar.searchingFor', { query: globalSearchQuery })}
                </div>
                <div className="flex items-center gap-1">
                  {([
                    { id: 'all', label: t('history.search.roleFilter.all'), icon: null },
                    { id: 'user', label: t('history.search.roleFilter.user'), icon: <UserIcon className="w-3 h-3" /> },
                    { id: 'assistant', label: t('history.search.roleFilter.assistant'), icon: <SparklesIcon className="w-3 h-3" /> }
                  ] as const).map((option) => (
                    <button
                      key={option.id}
                      type="button"
            ...
            {showInitialHistoryLoading ? (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-text-primary">
                          {t('history.sidebar.loading.title')}
                        </div>
                        <div className="mt-1 text-xs leading-5 text-text-tertiary">
                          {t('history.sidebar.loading.description')}
                        </div>
                      </div>
                    </div>
              ...
              isSearching ? (
                <div className="px-2.5 py-2 text-xs text-text-tertiary">{t('history.sidebar.searching')}</div>
              ) : groupedSearchResults.length === 0 ? (
                <div className="px-2.5 py-2 text-xs text-text-tertiary">{t('history.sidebar.noSearchResults')}</div>
              ) : (
              ...
              <div className="px-2.5 py-2 text-xs text-text-tertiary">
                {t('history.sidebar.noHistoryFound')}
              </div>
            ) : (
